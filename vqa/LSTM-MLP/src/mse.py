...
def main():
    ...
    parser.add_argument('--num-epochs', type=int, default=20, metavar='<num-epochs>')
    ...
    model_filename = 'models/mse_lstm_units_%i_layers_%i_mlp_units_%i_layers_%i_%s_lr%.1e_dropout%.2f' % (args.lstm_hidden_units, args.lstm_hidden_layers, args.mlp_hidden_units, args.mlp_hidden_layers, args.mlp_activation, args.learning_rate, args.dropout)
    ...
    for k in range(args.num_epochs):
        ...
        for i in range(len(dev_question_batches)):
            ...
            for j in range(5):
                similarity[j] = np.diag(pairwise_distances(prob, choice_feats[j], metric='euclidean'))
