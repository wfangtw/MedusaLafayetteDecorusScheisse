...
def main():
    ...
    model = Sequential()
    model.add(Merge(
        [language_model, caption_model], mode='concat', concat_axis=1
        ))
    ...
    model_filename = 'models/captiononly.lstm_units_%i_layers_%i.mlp_units_%i_layers_%i_%s.lr%.1e.dropout%.2f' % (args.lstm_hidden_units, args.lstm_hidden_layers, args.mlp_hidden_units, args.mlp_hidden_layers, args.mlp_activation, args.learning_rate, args.dropout)
    ...
    for k in range(args.num_epochs):
        ...
        for i in train_indices:
            ...
            loss = model.train_on_batch([X_question_batch, X_caption_batch], Y_answer_batch)
            loss = loss[0].tolist()
        ...
        for i in range(len(dev_question_batches)):
            ...
            prob = model.predict_proba([X_question_batch, X_caption_batch], args.batch_size, verbose=0)
