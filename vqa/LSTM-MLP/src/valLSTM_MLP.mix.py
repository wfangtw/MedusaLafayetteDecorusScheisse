...
def main():
    ...
    parser.add_argument('--model-2feats', type=str, required=True, metavar='<model-path>')
    parser.add_argument('--weights-2feats', type=str, required=True, metavar='<weights-path>')
    ...
    model_2feats = model_from_json(open(args.model_2feats,'r').read())
    model_2feats.compile(loss='categorical_crossentropy', optimizer='rmsprop')
    model_2feats.load_weights(args.weights_2feats)
    ...
    for i in range(len(dev_question_batches)):
        ...
        prob_inc = model_2feats.predict_proba([X_question_batch, X_vgg_image_batch, X_inc_image_batch], batch_size, verbose=0)
        prob = (prob_vgg + prob_inc)/2
