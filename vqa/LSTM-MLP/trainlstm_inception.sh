...
log_path=$(printf 'accuracy/inception.lstm.%s.%s.mlp.%s.%s.lrn2e-4.relu.dropout_%s.log' "$lstm_layers" "$lstm_units" "$mlp_layers" "$mlp_units" "$dropout")
#echo $log

python src/trainLSTM_MLP.inception.py \
