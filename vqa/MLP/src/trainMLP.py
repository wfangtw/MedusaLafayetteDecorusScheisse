...
#   PackageName:    [ MLP ]                                                          #
#   Synopsis:       [ Train MLP framework for visual question answering ]            #
...
from utils import  LoadIds, LoadAnswers, LoadChoices, LoadVGGFeatures, LoadGloVe, GetImagesMatrix, GetAnswersMatrix, GetChoicesTensor, MakeBatches, InterruptHandler
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='trainMLP.py',
    ...
    model = Sequential()
    model.add(Dense(
        output_dim=args.mlp_hidden_units, input_dim=img_dim
        ))
    ...
    for k in range(args.num_epochs):
        ...
        for i in pbar(range(len(dev_image_batches))):
            ...
            prob = model.predict_proba(X_image_batch, args.batch_size, verbose=0)
