...
def GetQuestionsMatrix(questions, word_embedding, word_map):
    batch_size = len(questions)
    word_vec_dim = 300
    questions_matrix = np.zeros((batch_size, word_vec_dim), float)
    for i in range(len(questions)):
        tokens = questions[i]
        for j in range(len(tokens)):
            questions_matrix[i] += GetWordFeature(tokens[j], word_embedding, word_map)
        questions_matrix[i] /= len(tokens)
