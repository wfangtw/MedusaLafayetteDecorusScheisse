######################################################################################
#   FileName:       [ trainWordVec_MLP.py ]                                          #
#   PackageName:    [ WordVec-MLP ]                                                  #
#   Synopsis:       [ Train WordVec-MLP framework for visual question answering ]    #
...
from utils import  LoadIds, LoadQuestions, LoadAnswers, LoadChoices, LoadVGGFeatures, LoadGloVe, GetImagesMatrix, GetQuestionsMatrix, GetAnswersMatrix, GetChoicesTensor, MakeBatches, InterruptHandler
...
def main():
    ...
    model = Sequential()
    model.add(Dense(
        output_dim=args.mlp_hidden_units, input_dim=(word_vec_dim+img_dim)
        ))
    ...
    model_filename = 'models/mlp_units_%i_layers_%i' % (args.mlp_hidden_units, args.mlp_hidden_layers)
    ...
    for k in range(args.num_epochs):
        ...
        for i in train_indices:
            X_question_batch = GetQuestionsMatrix(train_question_batches[i], word_embedding, word_map)
        ...
        for i in pbar(range(len(dev_question_batches))):
            # feed forward
            X_question_batch = GetQuestionsMatrix(dev_question_batches[i], word_embedding, word_map)
