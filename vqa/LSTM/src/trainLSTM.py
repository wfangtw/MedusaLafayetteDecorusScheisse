...
#   PackageName:    [ LSTM ]                                                         #
#   Synopsis:       [ Train LSTM framework for visual question answering ]           #
...
from utils import  LoadIds, LoadQuestions, LoadAnswers, LoadChoices, LoadGloVe, GetQuestionsTensor, GetAnswersMatrix, GetChoicesTensor, MakeBatches, InterruptHandler
...
def main():
    ...
    for i in range(args.lstm_hidden_layers-1):
        model.add(LSTM(
    ...
    for k in range(args.num_epochs):
        ...
        for i in train_indices:
            ...
            loss = model.train_on_batch(X_question_batch, Y_answer_batch)
            loss = loss[0].tolist()
        ...
        for i in pbar(range(len(dev_question_batches))):
            ...
            prob = model.predict_proba(X_question_batch, args.batch_size, verbose=0)
