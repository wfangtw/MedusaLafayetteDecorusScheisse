...
class HiddenLayer:
    ...
    lin_output = relu(T.dot(self.W, x) + self.b.dimshuffle(0, 'x'))
    self.output = (
...
class MLP:
    def __init__(self, input, n_in, n_hidden, n_out, n_layers):
        ...
        self.params.extend(hiddenLayers[0].params)
        self.velo.extend(hiddenLayers[0].velo)
        for i in range(1, n_layers):
            ...
            self.params.extend(hiddenLayers[i].params)
            self.velo.extend(hiddenLayers[i].velo)
        # output layer
        ...
        self.params.extend(logRegressionLayer.params)
        self.velo.extend(logRegressionLayer.velo)
        # L1 regularization
        l1_sum = 0
