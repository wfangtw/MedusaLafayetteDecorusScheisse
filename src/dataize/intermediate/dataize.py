...
for flabel in label_dim:
    dim_195 = []
    for x in range(4):
        dim_195.extend(label_dim[flabel][0])
    for x in range(1):
        dim_195.extend(label_dim[flabel][1+x])
    for x in range(len(label_dim[flabel])):
        if x >= len(label_dim[flabel])-2:
            dim_195.extend(label_dim[flabel][-1])
        else:
            dim_195.extend(label_dim[flabel][x+2])
        for y in range(39):
            dim_195.pop(0)

        dim_195 = map(float, dim_195)
        if flabel.split('_')[0] in label_dev:
            dev_ip.append(dim_195)
            dev_op.append(int(label[flabel][x]))
        else:
            train_ip.append(dim_195)
...
with open("../../../training_data/intermediate/train.in", "w") as f_train:
    #f_train.write(str(train))
    cPickle.dump(train, f_train)
with open("../../../training_data/intermediate/dev.in", "w") as f_dev:
    #f_dev.write(str(dev))
